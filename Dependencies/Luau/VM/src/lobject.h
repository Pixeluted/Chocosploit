...
#include "lua.h"
#include "lcommon.h"
#include "../../../../ShufflesAndVmValues.hpp"
...
typedef struct Udata
{
    ...
    UserdataMeta<struct LuaTable*> metatable;
...
typedef struct Proto
{
    ...
    VM_SHUFFLE5(
        ;,
        ProtoMember<uint8_t*> lineinfo,      // for each instruction, line number as a delta from baseline
        ProtoMember<int*> abslineinfo,       // baseline line info, one entry for each 1<<linegaplog2 instructions; allocated after lineinfo
        ProtoMember<struct LocVar*> locvars, // information about local variables
        ProtoMember<TString**> upvalues,     // upvalue names
        ProtoMember<TString*> source
    );

    ProtoDebugName<TString*> debugname;
    ProtoDebugInsn<uint8_t*> debuginsn; // a copy of code[] array with just opcodes

    ProtoTypeInfo<uint8_t*> typeinfo;
...
typedef struct Closure
{
    ...
    union
    {
        ...
        {
            lua_CFunction f;
            ClosureCont<lua_Continuation> cont;
            ClosureDebugname<const char*> debugname;
            TValue upvals[1];
        } c;
...
#define luaO_nilobject (reinterpret_cast<TValue*>(DummyNode))
